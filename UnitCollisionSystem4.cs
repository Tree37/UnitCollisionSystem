...
public class UnitCollisionSystem4 : SystemBase
{
    ...
    private EntityQuery frameQuery;
    ...
    protected override void OnUpdate()
    {
        ...
        //frameQuery = GetEntityQuery( typeof( Translation ) , typeof( TIndex ) );
        //frameQuery.SetSharedComponentFilter( new TIndex { Value = frameCount } );
        ...
        JobHandle broadphaseBarrier = JobHandle.CombineDependencies( 
            job.Schedule( generalQuery , Dependency ) , copyJob.Schedule( generalQuery , Dependency ) , recordJob.Schedule( generalQuery , Dependency ) );
    ...
    private struct BroadPhaseJob : IJobEntityBatchWithIndex
    {
        ...
        public void Execute( ArchetypeChunk batchInChunk , int batchIndex , int indexOfFirstEntityInQuery )
        {
            ...
            for ( int i = 0; i < batchInChunk.Count; i++ )
            {
                ...
                while ( grid[ gridIndex + count ] != 0 && count < CELL_CAPACITY )
                {
                    count++;
                }
    ...
    {
        [WriteOnly] public NativeArray<CopyUnitData> copyArray;
